...
namespace core::amqp {
    namespace impl {
        ...
        AmqpConnectionImpl::AmqpConnectionImpl(Configurations *configs, shared_ptr<AMQP::TcpHandler> handler)
                : handler_(handler) {
//    AMQP::LibBoostAsioHandler handler(*context->get_service());
            ...
            auto addr = AMQP::Address(configs->get("amqp-host"), configs->get<int>("amqp-port"), login,
                                      configs->get("amqp-vhost"));
